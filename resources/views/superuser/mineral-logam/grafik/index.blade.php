<x-layouts.superuser title="Grafik dan Visualisasi">
    <div class="flex justify-between items-end mb-4">
        <div>
            ...
            <p class="text-xs text-gray-500">
                ...
                @else
                    Perbandingan Antar Komoditas — Mineral Logam
                @endif
            </p>
        </div>
        <form method="GET">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5 mb-4">
        <div class="text-sm font-semibold mb-4">
            @if($komoditasTerpilih)
                Total Sumber Daya per Provinsi — {{ $komoditasTerpilih->nama_komoditas }} (Ton Bijih)
            @else
                Total Sumber Daya per Komoditas (Ton Bijih)
            @endif
        </div>
            ...
            <div class="flex gap-4 mt-2 text-[10px] text-gray-400">
                @foreach($perProvinsi as $row)
                    <div class="flex-1 text-center">
                        {{ $komoditasTerpilih ? ($row->provinsi->nama_provinsi ?? '-') : ($row->komoditasLogam->nama_komoditas ?? '-') }}
                    </div>
                @endforeach
            </div>
        @else
