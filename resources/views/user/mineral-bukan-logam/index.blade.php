<x-layouts.user title="Data Bukan Logam & Batuan">
    <h1 class="text-xl font-bold mb-1">Data Mineral Bukan Logam & Batuan</h1>
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-4">
        ...
        <select name="komoditas_bukan_logam_id" class="border-gray-300 rounded-md text-sm">
            <option value="">Semua Komoditas</option>
            @foreach(\App\Models\KomoditasBukanLogam::get() as $k)
                <option value="{{ $k->id }}" @selected(request('komoditas_bukan_logam_id')==$k->id)>{{ $k->nama_komoditas }}</option>
            @endforeach
        </select>
